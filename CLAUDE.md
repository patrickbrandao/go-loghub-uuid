...
**Language split (enforce when editing):** code *identifiers* (types, functions, variables, constants, package/file names) are in **English**; *comments*, the documentation under `docs/` and `skill/`, and the spec are in **Brazilian Portuguese**; human-facing *string literals* (error messages, console/report labels) are also Portuguese. No emojis anywhere — formal text only. So a typical function has an English name and a Portuguese doc comment; keep that pattern.
...
**`tests/compare/` is a nested module and must stay one.** It requires `github.com/google/uuid` to prove, in code, the v1 repetition that `docs/06-relogio-e-concorrencia.md` §4.2 and this file claim about that package. Having no dependencies is a property of this project, so the dependency is quarantined: `go test ./...` from the root does not descend into nested modules, and the root `go.mod` has no `require` and no `go.sum` at all. Verify with `go list -m all` at the root, which must print only this module. Do not move these tests up into `./tests/`, and do not add the comparison to the fast `test` CI job — it needs the network and a third party's release can't be allowed to break this library's CI. Scope is **behavior**, not speed: a comparative benchmark table was deliberately left out, see `CHANGELOG.md`. `tests/compare/golden_test.go` also reads the published v1/v2 golden vectors (`docs/09-vetores-dourados-e-apendice-rfc.md` §10 case 18) with google's parser, and measures that google's v1.6.0 UUIDv6 does not follow the RFC 9562 §5.6 field order, which is why the v6 vector has no external check.
...
- `construct.go` — building a UUIDv7 from an **explicit instant** instead of
  ...
  `docs/04-uuidv7-formato-e-niveis.md` §3.6.
- `bounds.go` — `MinAt`, `MaxAt`, `RangeAt`: the UUIDv7 that delimits an
  ...
  with the free bits held constant instead of drawn. See `docs/04-uuidv7-formato-e-niveis.md` §3.5.
...
- `compat.go` — aliases matching `github.com/google/uuid` names and signatures. See `docs/12-migracao-google-uuid.md`.

**Three invariants worth stating explicitly.** First, `String()` and `encodeHex` duplicate the same eight lines on purpose: `String()` is the hot path and must not pay a call. Second, `packV7` in `construct.go` duplicates the byte layout of `Generate` for the same reason; folding the two into one function would put a call or a branch in the hot path. Note the scope: **everything off the hot path shares `packV7`** — bounds and generation-at-instant both go through it — and only `Generate` keeps a private copy. Third, `FromString` still returns the bare `ErrInvalidFormat` sentinel, so `err == ErrInvalidFormat` keeps working for existing callers; only `Parse` returns the wrapped, more specific errors. All three are recorded in `docs/10-armadilhas-e-decisoes-de-projeto.md` §11 — do not "fix" any of them as a DRY or consistency finding.

**Settled decisions live in `docs/10-armadilhas-e-decisoes-de-projeto.md` §11.** Before proposing a change to the default entropy source, a monotonic counter, an injectable clock, the three invariants above, or a v1.0.0 tag, read that section: each was decided with a reason and a stated bar for reopening. Record every new design decision there — including refusals — plus a `CHANGELOG.md` entry. An unrecorded refusal comes back at the next audit.
...
Because the precision bits sit immediately after the milliseconds, **lexicographic string order stays chronological** across all levels (subject to the tie-break caveat above). Unknown `Level` values fall back to `Level1`. The exact byte layout is documented in the `Generate` doc comment at [uuid.go:154](uuid.go:154), in [STARTHERE.md](STARTHERE.md) §4 and in `docs/04-uuidv7-formato-e-niveis.md` §3.1, and it is **written a second time in code** by `packV7` in [construct.go](construct.go), which mirrors `Generate` field by field with the entropy arriving as parameters — keep all four in sync if the bit layout ever changes. That duplication is deliberate and recorded in `docs/10-armadilhas-e-decisoes-de-projeto.md` §11.2: the hot path must not pay a call to share it. `Generate` carries no pointer back, so a change there has to be followed into `construct.go` by hand. `TestGenerateAtMatchesGenerateLayout` is what catches the omission: it generates off the clock, reads the embedded instant back and regenerates for it, so the two copies must agree byte for byte.
...
**`Scan` treats empty text as absent, but does not match `google/uuid`'s handling of the destination.** `""` and `[]byte{}` set `Nil` without error (and `Valid=false` on `NullUUID`); migrated code reading `DEFAULT ''` columns depends on it. Where the two libraries diverge: `google/uuid`'s `Scan` returns `nil` on an absent value *without touching the destination*, while this library's overwrites it to `Nil`/`Valid=false`. Measured in `tests/compare`: scanning `nil`/`""`/`[]byte{}` into a receiver that already holds a value leaves `google.UUID` unchanged and zeroes this library's `UUID`. `docs/12-migracao-google-uuid.md` §1 documents the difference. `IsInvalidLengthError` uses `errors.Is`. `NullUUID.UnmarshalJSON` delegates to `encoding/json` only when the string contains a backslash; the no-escape path stays allocation-free.
...
`ImportBinary` is deliberately **level-blind**: it always reads `rand_a` as microseconds and the top 10 bits of `rand_b` as nanoseconds, treating random bits as if they were precise time (per spec). It cannot know which level produced a UUID. `TimestampWithLevel` is the deliberate opposite: it takes the level and **discards** any sub-millisecond field outside 0..999, and at Level3 both fields fall together (an out-of-range `rand_a` proves the top of `rand_b` is noise too). Both policies are normative in `docs/08-inspecao-serializacao-banco.md` §7 and recorded in `docs/10-armadilhas-e-decisoes-de-projeto.md` §11.3; do not unify them, and do not "fix" either as an inconsistency.

**Golden vectors.** `tests/golden_test.go` holds two contract tables from `docs/09-vetores-dourados-e-apendice-rfc.md` §10: the multilevel vectors (case 12, produced via `MinAt`/`MaxAt`) and the v1/v2/v6 vectors (case 18, fixed sequence `0x33c8`, node `02:11:22:33:44:55`). The second table fixes the *reader*; the *writer* is fixed by a test-local re-packer written from `docs/05-outras-versoes-uuid.md` §4.1, so a shift error applied symmetrically in `version1.go` and `inspect.go` no longer cancels out. Never fill a vector with this library's own output: the published values were computed from the formulas and checked against Python's stdlib (v1) and `google/uuid` (v1/v2 readers). `GregorianTime.UnixTime` uses euclidean division and returns a canonical pair (nsec in 0..999,999,999) also before 1970; `Time()` is unaffected, and `TestGregorianUnixTimeBeforeEpochIsCanonical` is the lock.
...
- [docs/INDEX.md](docs/INDEX.md) — index of the language-agnostic specification, sufficient to reimplement the entire library from scratch across all supported UUID versions (1 to 8, parsing, concurrency, and serialization), split into numbered files `01` to `13`. The original `SPEC.md` section numbers (§3.5, §11.2, ...) are preserved inside the files, so a bare section reference is still searchable. **[docs/10-armadilhas-e-decisoes-de-projeto.md](docs/10-armadilhas-e-decisoes-de-projeto.md) §11 is the settled-decisions register**: read it before proposing any design change, and add to it whenever a new decision is taken.
- [docs/12-migracao-google-uuid.md](docs/12-migracao-google-uuid.md) — moving from `github.com/google/uuid`; lists what was intentionally not imported (`SetRand`, the rand pool, `SetNodeInterface`) and why.
- [docs/13-processo-de-release.md](docs/13-processo-de-release.md) — release procedure: prerequisites, tag and `gh release`, post-publication check, and why tags are never moved.
...
- [skill/SKILL.md](skill/SKILL.md) — the Agent Skills package (`agentskills.io` format) that teaches AI coding agents how to consume the library: API surface, version/level choice, worked examples and pitfalls. It restates the public API, so any change to an exported name or signature must be mirrored there; it must not restate the spec, only point at `docs/`.
